...
if __name__=='__main__':
    ...
    img_list = []
    path_list = []
    for img_f in (dataset_path / 'train').iterdir():
        if not img_f.is_file():
            continue
        if img_f.suffix.lower() != '.jpg':
            continue
...
#             shutil.rmtree(str(moved_temp_keras))
        ...
        keras_test_dir = dataset_path / 'train_sketch'
        ...
        for img_fs in tqdm(list(chunks(bench_imgs, 8))):
            ...
            for img_f, sketch in zip(img_fs, krs):
                print(str(img_f))
                if(str(img_f)=='dataset/benchmark/.ipynb_checkpoints'):
                    continue
                sketch = add_intensity(sketch, 1.4)
