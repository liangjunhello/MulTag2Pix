...
# 是把需要的tag标签从json文件中提取出来
#  筛选大小
SIMPLE_BACKGROUND = 412368
WHITE_BACKGROUND = 515193
sketch_tags = [513837, 1931] # grayscale, sketch
include_tags = [1821,399836,412555,421662,449194,487156,479955,480899] # 2~5个boys and girls
# 470575, 540830
hair_tags = [87788, 16867, 13200, 10953, 16442, 11429, 15425, 8388, 5403, 16581, 87676, 16580, 94007, 403081, 468534,1419313]
eye_tags = [10959, 8526, 16578, 10960, 15654, 89189, 16750, 13199, 89368, 95405, 89228, 390186 ,517427]
# blacklist_tags = [63, 4751, 12650, 172609, 555246, 513475] # comic, photo, subtitled, english, black border, multiple_views
blacklist_tags = [63, 4751, 12650, 172609, 555246, 470575, 540830] # comic, photo, subtitled, english, black border, multiple_views
...
#         白色的背景
        ...
        if SIMPLE_BACKGROUND not in tags:
            return False
#         改动    1821,399836,412555,421662,449194,487156,479955,480899
        conditions = all(len(tags.intersection(lst)) == 1 for lst in [include_tags])
