...
# 处理一下benchmark的图片
def crop_benchmark(dataset_path,testset_path):
    # testset_path = './testset '
    save_base = testset_path/ 'benchmark'
    save_base1 = testset_path/ 'benchmark1'
    # if not save_base1.exists():
    #     save_base1.mkdir()
    ...
    for img_f in tqdm(img_list):
        ...
        # delete_img(img_f)
        cv2.imwrite(str(save_base1 / (img_f.stem + '.png')), cropped_img)
...
def upscale_lanczos_all(dataset_path, image_base, save_path):
    ...
    # temp_str='dataset/rgb_cropped/.ipynb_checkpoints'
    temp_str='dataset/keras_single/.ipynb_checkpoints'
...
def upscale_all(dataset_path, image_base=None, save_path=None):
    if image_base is None:
        image_base = dataset_path / 'keras_single'
    if save_path is None:
        save_path = dataset_path / 'keras_single_768'
...
if __name__=='__main__':
    ...
    parser.add_argument('--testset_path', type=str, default='./testset',
                        help='path to dataset directory')
    ...
    parser.add_argument('--crop_benchmark', action='store_true',
                        help='only makes crop_benchmark image')
    ...
    if args.download_waifu2x_caffe:
        ...
        #     crop_benchmark(dataset_path,testset_path)
